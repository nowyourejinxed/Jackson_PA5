...
bool Mesh::loadModelFromFile(const char* path) {
	...
	for (int i = 0; i < scene->mNumMeshes; i++) {
		...
		for (int j = 0; j < iMeshFaces; j++) {
			...
			for (int k = 0; k < 3; k++) {
				glm::vec3 vertex = glm::vec3(mesh->mVertices[face.mIndices[k]].x, mesh->mVertices[face.mIndices[k]].y, mesh->mVertices[face.mIndices[k]].z);
				...
				if (mesh->HasTextureCoords(0)) {
					texCoord = glm::vec2(mesh->mTextureCoords[0][face.mIndices[k]].x, mesh->mTextureCoords[0][face.mIndices[k]].y);
				}
